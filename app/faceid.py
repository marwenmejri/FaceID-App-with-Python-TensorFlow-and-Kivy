...
class CamApp(App):
    ...
    def update(self, *args):
        ...
        frame = frame[120:120+255, 200:200+250, :]
    ...
    def verify(self, *args):
        ...
        SAVE_PATH = os.path.join(
            'application_images', 'input_image', 'input_image.jpg')
        ret, frame = self.capture.read()
        frame = frame[120:120+255, 200:200+250, :]
        ...
        for image in os.listdir(os.path.join('application_images', 'verification_images')):
            input_img = self.preprocessing(os.path.join(
                'application_images', 'input_image', 'input_image.jpg'))
            inp_imgs.append(input_img)
            validation_img = self.preprocessing(os.path.join(
                'application_images', 'verification_images', image))
            val_imgs.append(validation_img)

        inp_imgs_tens = tf.stack(inp_imgs)
        val_imgs_tens = tf.stack(val_imgs)

        # Make Predictions
        results = self.model.predict([inp_imgs_tens, val_imgs_tens])

        # Detection Threshold: Metric above which a prediciton is considered positive
        detection = np.sum(np.array(results) > detection_threshold)

        # Verification Threshold: Proportion of positive predictions / total positive samples
        verification = detection / len(os.listdir(os.path.join('application_images',
                                                               'verification_images')))
